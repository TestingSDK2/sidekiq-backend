...
func ConvertAnyToMap(originalMap map[string]*anypb.Any) (map[string]interface{}, error) {
	convertedMap := make(map[string]interface{})
	for key, value := range originalMap {
		var interfaceValue interface{}
		err := json.Unmarshal(value.Value, &interfaceValue)
		if err != nil {
			return nil, err
		}
		if key == "_id" {
			obj, err := primitive.ObjectIDFromHex(interfaceValue.(string))
			if err != nil {
				return nil, err
			}
			convertedMap[key] = obj
		} else {
			convertedMap[key] = interfaceValue
		}
	}
