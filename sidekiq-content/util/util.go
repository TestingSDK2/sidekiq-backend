...
func MaptoPbAny(m interface{}) (*anypb.Any, error) {
	byteVal, err := json.Marshal(m)
	if err != nil {
		return nil, err
	}
	// var val *anypb.Any
...
func MaptoAny(m map[string]interface{}) (*anypb.Any, error) {
	byteVal, err := json.Marshal(m)
	if err != nil {
		return nil, err
	}
...
func SetGenericResponse(d map[string]interface{}) (*contentrpc.GenericResponse, error) {
	res := &contentrpc.GenericResponse{}
	v, err := MaptoPbAny(d["data"])
	if err != nil {
		return nil, err
	}
	res.Data = v
	res.Message = d["message"].(string)
	res.Status = int32(d["status"].(int))
	return res, nil
}

func MapToMapAny(m map[string]interface{}) (map[string]*anypb.Any, error) {
	ret := make(map[string]*anypb.Any)
	for k, v := range m {
		byteVal, err := json.Marshal(v)
		if err != nil {
			return nil, errors.Wrap(err, "unable to convert map to anypb.Any")
		}
		ret[k] = &anypb.Any{Value: byteVal}
	}
	return ret, nil
}
...
func BsonAtoStrArr(dat primitive.A) []string {
	ret := []string{}
	for _, val := range dat {
