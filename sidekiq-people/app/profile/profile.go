...
func updateProfileTagsNew(db *mongodatabase.DBConfig, mySql *database.Database, profileID string, profileTags []string) error {
...
func sendConnectionRequest(mysql *database.Database, db *mongodatabase.DBConfig, emailService email.Service,
	...
	cp, err := helper.GetConciseProfile(mysql, idInt, storageService)
	if err != nil {
...
func addConnectionDetails(mysql *database.Database, db *mongodatabase.DBConfig, payload model.Connection, storageService storage.Service) (map[string]interface{}, error) {
	...
	data["photo"], err = helper.GetProfileImage(mysql, storageService, 0, profileIDInt)
	if err != nil {
		fmt.Println("error in fetching profile photo")
	}
...
func getConnectionDetails(mysql *database.Database, db *mongodatabase.DBConfig, storageService storage.Service, callerProfileID, connectionProfileID string) (map[string]interface{}, error) {
	...
	connectionProfileIDInt, _ := strconv.Atoi(connectionProfileID)

	response.Photo, err = helper.GetProfileImage(mysql, storageService, 0, connectionProfileIDInt)
	if err != nil {
		response.Photo = ""
	}

	response.Thumbs, err = helper.GetProfileImageThumb(mysql, storageService, 0, connectionProfileIDInt)
	if err != nil {
		response.Thumbs = model.Thumbnails{}
	}
...
func fetchProfileView(storageService storage.Service, sql *database.Database, db *mongodatabase.DBConfig, profileID, myID string) (map[string]interface{}, error) {
	...
	photo, err := helper.GetProfileImage(sql, storageService, 0, profileInt)
	if err != nil {
...
func getOrgStaff(storageService storage.Service, db *database.Database, profileID int, accountID, limit, page, searchParameter string) (map[string]interface{}, error) {
	...
	for i := range staffProfiles {
		staffProfiles[i].Photo, err = helper.GetAccountImage(db, storageService, staffProfiles[i].Id, staffProfiles[i].Id)
		if err != nil {
			staffProfiles[i].Photo = ""
			fmt.Println("unable to fetch account photo")
		}

		staffProfiles[i].Thumbs, err = helper.GetAccountImageThumb(db, storageService, staffProfiles[i].Id)
		if err != nil {
...
func fetchStaffProfile(db *database.Database, accountID, comanagerID int, storageService storage.Service) (map[string]interface{}, error) {
	...
	profile.Photo, err = helper.GetAccountImage(db, storageService, comanagerID, 0)
	if err != nil {
		logrus.Println("unable to get account image")
		profile.Photo = ""
	}

	profile.Thumbs, err = helper.GetAccountImageThumb(db, storageService, comanagerID)
	if err != nil {
		profile.Thumbs = model.Thumbnails{}
...
func contains(db *database.Database, tableName, dbField, value string, isAccountID bool, accountID int) bool {
...
func getOrgInfo(db *database.Database, accountID int, storageService storage.Service) (map[string]interface{}, error) {
	...
	Info.Photo, err = helper.GetOrgImage(db, storageService, accountID)
	if err != nil {
		Info.Photo = ""
		logrus.Println("Organization image not get ", err)
	}

	Info.Thumbs, err = helper.GetOrgImageThumb(db, storageService, accountID)
	if err != nil {
...
func updateOrgInfo(db *database.Database, payload model.Organization) (map[string]interface{}, error) {
	...
	// check if org creds already exists
	if contains(db, "OrgProfile", "email", payload.Email, true, payload.AccountID) {
		return util.SetResponse(nil, 0, "Email already exists. Please use another email"), nil
	} else if contains(db, "OrgProfile", "registrationNumber", payload.RegistrationNumber, true, payload.AccountID) {
		return util.SetResponse(nil, 0, "Registration Number already exists. Please use another Registration Number"), nil
	} else if contains(db, "OrgProfile", "website", payload.Website, true, payload.AccountID) {
		return util.SetResponse(nil, 0, "Website address already exists. Please use another Website address"), nil
	} else if contains(db, "OrgProfile", "organizationName", payload.OrganizationName, true, payload.AccountID) {
		return util.SetResponse(nil, 0, "Organization name already exists. Please use another Organization name"), nil
	}
